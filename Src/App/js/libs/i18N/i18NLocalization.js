(function () {
	window.i18N = {
		changeLanguage: function (lang, translation) {
			var language = lang && translation && translation[lang];
			if (language) {
				i18N.lang = lang;
				var translations = $("*").contents().filter(function () { return this.nodeType === 3 })
				.filter(function () {
					var $this = $(this),
						text = $.trim(this._text || $this.text()),
						translation = language[text];
					if (!text)
						return false;
					this._text = text;
					translation = translation || (translation !== "" ? text : translation)
					this.textContent = translation;
					return true;
				});
				$("[data-translate]").each(function (index, elem) {
					var $elem = $(elem),
						attrs = ($elem.attr("data-translate") || '').split(",");
					$.each(attrs, function (index, attr) {
						index = $elem.attr(attr + "_text");
						if (!index) {
							index = $elem.attr(attr);
							$elem.attr(attr + "_text", index);
						}
						$elem.attr(attr, language[index] || index);
					});
				});
			}
		},
		noAccentRegex: make_pattern
	};
